...
public class BurnStacksMenu extends ListMenu {
  ...
  public BurnStacksMenu(final BattleHud hud, final PlayerBattleEntity activePlayer, final ListPosition lastPosition, final Runnable onClose) {
  ...
  protected void drawListEntry(final int index, final int x, final int y, final int trim) {
    ...
    renderText("Burn Stacks: " + DragoonModifier.dartBurnStacks[this.player.typeBentSlot_276], x, y, this.fontOptions);
  }
  ...
  protected void onUse(final int index) {
    if(DragoonModifier.dartBurnStacks[this.player.typeBentSlot_276] > 0) {
      DragoonModifier.dartBurnStackMode[this.player.typeBentSlot_276] = !DragoonModifier.dartBurnStackMode[this.player.typeBentSlot_276];
      if(DragoonModifier.dartBurnStacks[this.player.typeBentSlot_276] == DragoonModifier.DART_BURN_STACKS_MAX) {
        if(DragoonModifier.dartBurnStackMode[this.player.typeBentSlot_276]) {
          DragoonModifier.dartPreviousMp[this.player.typeBentSlot_276] = this.player.stats.getStat(MP_STAT.get()).getCurrent();
          this.player.stats.getStat(MP_STAT.get()).setCurrent(this.player.stats.getStat(MP_STAT.get()).getMax());
        } else {
          this.player.stats.getStat(MP_STAT.get()).setCurrent(DragoonModifier.dartPreviousMp[this.player.typeBentSlot_276]);
        }
      }
    }
    this.flags_02 &= ~0x8;
  ...
  public void draw() {
    ...
    renderText(DragoonModifier.dartBurnStackMode[this.player.typeBentSlot_276] ? "Activated" : "Deactivated", 160, 157, this.fontOptions);
  }
}
