...
public class WinglyMagicMenu extends ListMenu {
  ...
  public WinglyMagicMenu(final BattleHud hud, final PlayerBattleEntity activePlayer, final ListPosition lastPosition, final Runnable onClose) {
  ...
  protected void drawListEntry(final int index, final int x, final int y, final int trim) {
    ...
    renderText("Enhanced Magic", x, y, this.fontOptions);
  }
  ...
  protected void onUse(final int index) {
    if(DragoonModifier.meruWinglyMagic[this.player.typeBentSlot_276]) {
      DragoonModifier.meruWinglyMagic[this.player.typeBentSlot_276] = false;
      this.player.stats.getStat(MP_STAT.get()).setCurrent(this.player.stats.getStat(MP_STAT.get()).getCurrent() + 10);
      this.player.stats.getStat(GUARD_HEAL_STAT.get()).setRaw(10);
    } else {
      if(this.player.stats.getStat(MP_STAT.get()).getCurrent() >= 10) {
        DragoonModifier.meruWinglyMagic[this.player.typeBentSlot_276] = true;
        this.player.stats.getStat(MP_STAT.get()).setCurrent(this.player.stats.getStat(MP_STAT.get()).getCurrent() - 10);
        this.player.stats.getStat(GUARD_HEAL_STAT.get()).setRaw(15);
      }
    }
    this.flags_02 &= ~0x8;
    this.menuState_00 = 8;
  ...
  public void draw() {
    ...
    renderText(DragoonModifier.meruWinglyMagic[this.player.typeBentSlot_276] ? "Activated" : "Deactivated", 160, 157, this.fontOptions);
  }
}
