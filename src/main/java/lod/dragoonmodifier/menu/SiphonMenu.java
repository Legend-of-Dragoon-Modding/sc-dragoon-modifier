...
public class SiphonMenu extends ListMenu {
  ...
  public SiphonMenu(final BattleHud hud, final PlayerBattleEntity activePlayer, final ListPosition lastPosition, final Runnable onClose) {
  ...
  protected void drawListEntry(final int index, final int x, final int y, final int trim) {
    ...
    renderText("Siphon: " + DragoonModifier.roseSiphon[this.player.typeBentSlot_276] + '/' + DragoonModifier.roseSiphonMax[this.player.typeBentSlot_276], x, y, this.fontOptions);
  }
  ...
  protected void onUse(final int index) {
    if(DragoonModifier.roseSiphon[this.player.typeBentSlot_276] > 0) {
      DragoonModifier.roseSiphonActivated[this.player.typeBentSlot_276] = !DragoonModifier.roseSiphonActivated[this.player.typeBentSlot_276];
    }
    this.flags_02 &= ~0x8;
    this.menuState_00 = 8;
  }
