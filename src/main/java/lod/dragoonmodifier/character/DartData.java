...
public class DartData extends CharacterData2c {
  private final Map<RegistryId, CharacterSpellInfo> divineSpells = new HashMap<>();

  public DartData(final GameState52c gameState, final CharacterTemplate template, final StatCollection stats) {
    super(gameState, template, stats);

    this.addDivineSpell(REGISTRIES.spells.getEntry(DragoonModifier.MOD_ID, "divine_dg_ball").getId(), new CharacterSpellInfo(List.of(), UnlockState.UNLOCKED, 0));
    this.addDivineSpell(REGISTRIES.spells.getEntry(DragoonModifier.MOD_ID, "divine_dg_cannon").getId(), new CharacterSpellInfo(List.of(), UnlockState.UNLOCKED, 0));
  }
